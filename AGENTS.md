...
- Keep dependencies minimal. Each new library needs a reason in a comment next to it. The plan calls for ML Kit Text Recognition v2 (Chinese, bundled on-device model) and a lightweight pinyin library with word segmentation.
...
- **On-device only.** Never add the `INTERNET` permission, analytics, crash reporting, or any network client without explicit approval.
...
- Only `BuildConfig.DEBUG` builds may write a screenshot to disk, and only to app-private storage (`DebugScreenshotStore`). Release builds never persist screen content.
...
```
com.rubify
  service/   AccessibilityService: button callback, wiring only
  capture/   takeScreenshot() into a software bitmap
  ocr/       ML Kit wrapper that returns per-character boxes     (phase 3)
...
- No heavy work on the main thread. Capture callbacks run on `rubify-capture`, and OCR runs asynchronously.
